...
def main_glove():
	...
	C = np.asarray(compute_cooccurrence_matrix(dataset_text, vocab))
	...
	maxEpoch = 30		# maximum number of epochs
	learningRate = 1e-1 # learning rate / step size for SGD
	clip = 50			# gradient clip value
	m = 0.3				# moment parameter

	# Initialize word vectors randomly
	wordvecs = np.random.rand(vocab.size, d)
	contextvecs = np.random.rand(vocab.size, d)
	
	# Initalize biases to approximate occurrence frequency
	...
	for epoch in range(maxEpoch):
		...
		for b in range(idx.shape[0]//B+1):
			...
			w_i_inner_prod_w_j = np.sum(np.multiply(w_batch, c_batch), axis=1)[:,np.newaxis]
			b_i = wordbiases[i]
			b_j = contextbiases[j]
			log_C = np.log(C[i,j])[:,np.newaxis]
			error = (w_i_inner_prod_w_j + b_i + b_j - log_C)
			...
			fval_d = np.repeat(fval, d, axis=1)
			error_d = np.repeat(error, d, axis=1)
			wordvecs_grad = 2 * np.multiply(fval_d, np.multiply(c_batch, error_d))
			wordbiases_grad = 2 * np.multiply(fval, error)
			contextvecs_grad = 2 * np.multiply(fval_d, np.multiply(w_batch, error_d))
			contextbiases_grad = 2 * np.multiply(fval, error)
	...
	plot_word_vectors_tsne(wordvecs, vocab, file_name="glove_tsne_matrix.npz")
