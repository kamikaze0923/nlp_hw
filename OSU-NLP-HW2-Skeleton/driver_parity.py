...
class ParityLSTM(torch.nn.Module) :
    ...
    def __init__(self, hidden_dim=1) :
        ...
        self.h_0 = torch.nn.Parameter(torch.zeros(size=(self.lstm.num_layers, hidden_dim)))
        self.c_0 = torch.nn.Parameter(torch.zeros(size=(self.lstm.num_layers, hidden_dim)))
    ...
    def forward(self, x, s):
        assert len(s) == x.size()[0]
        h_0 = self.h_0.unsqueeze(1).expand(-1, len(s), -1) # add batch dimension and repeat
        c_0 = self.c_0.unsqueeze(1).expand(-1, len(s), -1) # add batch dimension and repeat
        x = x.unsqueeze(-1) # extend a feature dimension for x
        # _, (h_t, _) = self.lstm(x, (h_0, c_0))  # feed x to LSTM will result in feeding in the padded 0s(duplicate examples)
        x_pack = pack_padded_sequence(x, s, batch_first=True, enforce_sorted=False)
        _, (h_t, _) = self.lstm(x_pack, (h_0, c_0)) # take the last state from h_t, skip the unpack operation on the output
        return F.softmax(self.fc(h_t[-1]), dim=-1)
...
def runParityExperiment(model, max_train_length):
    logging.info("Starting parity experiment with model: " + model.__str__())
    lengths = []
    accuracy  = []
...
def train_model(model, train_loader, epochs=8000, lr=0.003):
