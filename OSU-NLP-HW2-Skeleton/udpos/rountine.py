...
def routine_loss(logits, label, x_lens, criterion=torch.nn.BCELoss(reduction='none')):
    ...
    loss = criterion(logits, pred_label_one_hot).sum()
...
def routine(dataloader, model, optimizer=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if optimizer:
            optimizer.zero_grad()
            out = model(batch)
            loss, batch_correct_word, batch_n_word = routine_loss(out, yy_pad[:,1:], x_lens)# match labels except <SOS>
            loss.backward()
    ...
    return avg_loss / n_words, correct_words / n_words, (correct_words, n_words)
